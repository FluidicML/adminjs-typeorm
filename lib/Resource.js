...
export class Resource extends BaseResource {
    ...
    databaseName() {
        return (this.model.getRepository().metadata.connection.options
            .database || "typeorm");
    }
    databaseType() {
    ...
    async count(filter) {
        return this.repo.count({ where: convertFilter(filter) });
    }
    async find(filter, 
    ...
    async findOne(id) {
        ...
        const instance = await this.repo.findOneBy(reference);
        if (!instance) {
            return null;
        }
    ...
    async findMany(ids) {
        ...
        const instances = await this.repo.findBy(reference);
        return instances.map((instance) => new BaseRecord(instance, this));
    }
    async create(params) {
        const unflattenedParams = flat.unflatten(this.prepareParams(params));
        const instance = this.repo.create(unflattenedParams);
